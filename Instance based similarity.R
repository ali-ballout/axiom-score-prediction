...
d <- read.table("scored axioms for instance based similarity.txt", header = TRUE)
...
endpoint <- "http://134.59.130.136:8890/sparql"
options <- "output=XML"
useragent <- paste("test", R.version.string)

# Define a list of common prefixes
prefixx <- c("owl","http://www.w3.org/2002/07/owl#",
  ...
  if(is.na(denom[i, length(d$axiom)]))
  {
    ...
    for(j in (i + 1):length(d$axiom))
      if(is.na(denom[i, j]))
      {
        ...
        qd <- SPARQL(endpoint, paste(prefix, query), ns = prefixx)
